...
class MainActivity : AppCompatActivity() {
    ...
    private fun getJsonData(lat: String, long: String) {
        val API_KEY = "a4a8993271ad47afb3ac8003707e4261"
        val queue = Volley.newRequestQueue(this)
        val cityName = getCityName(lat.toDouble(), long.toDouble())

        val url = "https://api.weatherbit.io/v2.0/current?lat=${lat}&lon=${long}&key=${API_KEY}&include=minutely"
    ...
    private fun setValues(response: JSONObject) {
        ...
        cityName.text = "City Name: " + response.getString("name")
        cityName.visibility = View.VISIBLE
        var tempr = response.getJSONObject("main").getString("temp")
        temperature.text = "Temparature: ${(tempr.toFloat() - 273).toInt()}°C"
